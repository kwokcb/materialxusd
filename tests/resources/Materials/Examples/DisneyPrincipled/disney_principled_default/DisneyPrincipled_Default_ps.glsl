...
uniform vec3 SR_default_baseColor = vec3(0.160000, 0.160000, 0.160000);
uniform float SR_default_metallic = 0.000000;
uniform float SR_default_roughness = 0.500000;
uniform float SR_default_anisotropic = 0.000000;
uniform float SR_default_specular = 0.500000;
uniform float SR_default_specularTint = 0.000000;
uniform float SR_default_sheen = 0.000000;
uniform float SR_default_sheenTint = 0.500000;
uniform float SR_default_clearcoat = 0.000000;
uniform float SR_default_clearcoatGloss = 1.000000;
uniform float SR_default_specTrans = 0.000000;
uniform float SR_default_ior = 1.500000;
uniform float SR_default_subsurface = 0.000000;
uniform vec3 SR_default_subsurfaceDistance = vec3(1.000000, 1.000000, 1.000000);
...
void NG_disney_principled(vec3 baseColor, float metallic, float roughness, float anisotropic, float specular, float specularTint, float sheen, float sheenTint, float clearcoat, float clearcoatGloss, float specTrans, float ior, float subsurface, vec3 subsurfaceDistance, out surfaceshader out1)
{
    ...
    surfaceshader surface_constructor_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            mx_generalized_schlick_bsdf(closureData, coat_intensity_out, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 5.000000, coat_roughness_dual_out, 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 0, coat_bsdf_out);
            BSDF metallic_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf(closureData, 1.000000, baseColor, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 5.000000, specular_roughness_out, 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 0, metallic_bsdf_out);
            BSDF dielectric_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf(closureData, dielectric_intensity_out, dielectric_tint_out, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 5.000000, specular_roughness_out, 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 0, dielectric_bsdf_out);
            BSDF transmission_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, 1.000000, baseColor, ior, vec2(0.000000, 0.000000), 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 1, transmission_bsdf_out);
            ...
            mx_subsurface_bsdf(closureData, 1.000000, baseColor, subsurfaceDistance, 0.000000, geomprop_Nworld_out1, subsurface_bsdf_out);
            BSDF diffuse_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_burley_diffuse_bsdf(closureData, invert_metalness_out, baseColor, roughness, geomprop_Nworld_out1, diffuse_bsdf_out);
            BSDF subsurface_mix_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, subsurface_bsdf_out, diffuse_bsdf_out, subsurface, subsurface_mix_out);
            ...
            mx_mix_bsdf(closureData, transmission_bsdf_out, sheen_layer_out, specTrans, transmission_mix_out);
            BSDF dielectric_layer_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, dielectric_bsdf_out, transmission_mix_out, dielectric_layer_out);
            BSDF metallic_mix_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, metallic_bsdf_out, dielectric_layer_out, metallic, metallic_mix_out);
            BSDF coat_layer_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, coat_bsdf_out, metallic_mix_out, coat_layer_out);

            surface_constructor_out.color += occlusion * coat_layer_out.response;
        }
...
void main()
{
    ...
    NG_disney_principled(SR_default_baseColor, SR_default_metallic, SR_default_roughness, SR_default_anisotropic, SR_default_specular, SR_default_specularTint, SR_default_sheen, SR_default_sheenTint, SR_default_clearcoat, SR_default_clearcoatGloss, SR_default_specTrans, SR_default_ior, SR_default_subsurface, SR_default_subsurfaceDistance, SR_default_out);
    material DisneyPrincipled_Default_out = SR_default_out;
    out1 = vec4(DisneyPrincipled_Default_out.color, 1.0);
}
