...
uniform sampler2D image_basecolor_file;
uniform vec4 image_basecolor_default = vec4(0.000000, 0.000000, 0.000000, 0.000000);
uniform vec2 image_basecolor_pivot = vec2(0.000000, 1.000000);
uniform vec2 image_basecolor_scale = vec2(1.000000, 1.000000);
uniform float image_basecolor_rotate = 0.000000;
uniform vec2 image_basecolor_offset = vec2(0.000000, 0.000000);
uniform int image_basecolor_operationorder = 1;
uniform int image_basecolor_uaddressmode = 2;
uniform int image_basecolor_vaddressmode = 2;
uniform int image_basecolor_filtertype = 1;
uniform vec4 image_basecolor_color = vec4(1.000000, 1.000000, 1.000000, 1.000000);
uniform vec4 image_basecolor_geomcolor = vec4(1.000000, 1.000000, 1.000000, 1.000000);
uniform sampler2D image_orm_file;
uniform vec3 image_orm_default = vec3(0.000000, 0.000000, 0.000000);
uniform vec2 image_orm_pivot = vec2(0.000000, 1.000000);
uniform vec2 image_orm_scale = vec2(1.000000, 1.000000);
uniform float image_orm_rotate = 0.000000;
uniform vec2 image_orm_offset = vec2(0.000000, 0.000000);
uniform int image_orm_operationorder = 0;
uniform int image_orm_uaddressmode = 2;
uniform int image_orm_vaddressmode = 2;
uniform int image_orm_filtertype = 1;
uniform sampler2D image_normal_file;
uniform vec3 image_normal_default = vec3(0.500000, 0.500000, 1.000000);
uniform vec2 image_normal_pivot = vec2(0.000000, 1.000000);
uniform vec2 image_normal_scale = vec2(1.000000, 1.000000);
uniform float image_normal_rotate = 0.000000;
uniform vec2 image_normal_offset = vec2(0.000000, 0.000000);
uniform int image_normal_operationorder = 0;
uniform int image_normal_uaddressmode = 2;
uniform int image_normal_vaddressmode = 2;
uniform int image_normal_filtertype = 1;
uniform sampler2D image_emission_file;
uniform vec3 image_emission_factor = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 image_emission_default = vec3(0.000000, 0.000000, 0.000000);
uniform vec2 image_emission_pivot = vec2(0.000000, 1.000000);
uniform vec2 image_emission_scale = vec2(1.000000, 1.000000);
uniform float image_emission_rotate = 0.000000;
uniform vec2 image_emission_offset = vec2(0.000000, 0.000000);
uniform int image_emission_operationorder = 0;
uniform int image_emission_uaddressmode = 2;
uniform int image_emission_vaddressmode = 2;
uniform int image_emission_filtertype = 1;
uniform float SR_boombox_transmission = 0.000000;
uniform float SR_boombox_specular = 1.000000;
uniform vec3 SR_boombox_specular_color = vec3(1.000000, 1.000000, 1.000000);
uniform float SR_boombox_ior = 1.500000;
uniform int SR_boombox_alpha_mode = 0;
uniform float SR_boombox_alpha_cutoff = 0.500000;
uniform float SR_boombox_iridescence = 0.000000;
uniform float SR_boombox_iridescence_ior = 1.300000;
uniform float SR_boombox_iridescence_thickness = 100.000000;
uniform vec3 SR_boombox_sheen_color = vec3(0.000000, 0.000000, 0.000000);
uniform float SR_boombox_sheen_roughness = 0.000000;
uniform float SR_boombox_clearcoat = 0.000000;
uniform float SR_boombox_clearcoat_roughness = 0.000000;
uniform float SR_boombox_emissive_strength = 1.000000;
uniform float SR_boombox_thickness = 0.000000;
uniform float SR_boombox_attenuation_distance = 0.0;
uniform vec3 SR_boombox_attenuation_color = vec3(1.000000, 1.000000, 1.000000);
...
void NG_gltf_image_color4_color4_1_0(sampler2D file, vec4 factor, vec4 default1, vec2 texcoord1, vec2 pivot, vec2 scale, float rotate, vec2 offset, int operationorder, int uaddressmode, int vaddressmode, int filtertype, out vec4 out1)
{
    const vec2 invert_scale_in1_tmp = vec2(1.000000, 1.000000);
    vec2 invert_scale_out = invert_scale_in1_tmp / scale;
    const float negate_rotate_in2_tmp = -1.000000;
    float negate_rotate_out = rotate * negate_rotate_in2_tmp;
    const vec2 negate_offset_in2_tmp = vec2(-1.000000, 1.000000);
    vec2 negate_offset_out = offset * negate_offset_in2_tmp;
    vec2 place2d_out = vec2(0.0);
    NG_place2d_vector2(texcoord1, pivot, invert_scale_out, negate_rotate_out, negate_offset_out, operationorder, place2d_out);
    vec4 image_out = vec4(0.0);
    mx_image_color4(file, 0, default1, place2d_out, uaddressmode, vaddressmode, filtertype, 0, 0, 0, vec2(1.000000, 1.000000), vec2(0.000000, 0.000000), image_out);
    vec4 scale_image_out = factor * image_out;
    out1 = scale_image_out;
}
...
void NG_gltf_colorimage(sampler2D file, vec4 default1, vec2 texcoord1, vec2 pivot, vec2 scale, float rotate, vec2 offset, int operationorder, int uaddressmode, int vaddressmode, int filtertype, vec4 color1, vec4 geomcolor, out vec3 outcolor, out float outa)
{
    vec4 image_out = vec4(0.0);
    NG_gltf_image_color4_color4_1_0(file, vec4(1.000000, 1.000000, 1.000000, 1.000000), default1, texcoord1, pivot, scale, rotate, offset, 0, 2, 2, filtertype, image_out);
    vec4 modulate_color_out = color1 * image_out;
    vec4 modulate_geomcolor_out = modulate_color_out * geomcolor;
    float separate_color_outr = 0.0;
    float separate_color_outg = 0.0;
    float separate_color_outb = 0.0;
    float separate_color_outa = 0.0;
    NG_separate4_color4(modulate_geomcolor_out, separate_color_outr, separate_color_outg, separate_color_outb, separate_color_outa);
    vec3 combine_color_out = vec3(separate_color_outr,separate_color_outg,separate_color_outb);
    float separate_alpha_out = separate_color_outa;
    outcolor = combine_color_out;
    outa = separate_alpha_out;
}
...
void NG_gltf_image_vector3_vector3_1_0(sampler2D file, vec3 default1, vec2 texcoord1, vec2 pivot, vec2 scale, float rotate, vec2 offset, int operationorder, int uaddressmode, int vaddressmode, int filtertype, out vec3 out1)
{
    const vec2 invert_scale_in1_tmp = vec2(1.000000, 1.000000);
    vec2 invert_scale_out = invert_scale_in1_tmp / scale;
    const float negate_rotate_in2_tmp = -1.000000;
    ...
    vec2 place2d_out = vec2(0.0);
    NG_place2d_vector2(texcoord1, pivot, invert_scale_out, negate_rotate_out, negate_offset_out, operationorder, place2d_out);
    vec3 image_out = vec3(0.0);
    mx_image_vector3(file, 0, default1, place2d_out, uaddressmode, vaddressmode, filtertype, 0, 0, 0, vec2(1.000000, 1.000000), vec2(0.000000, 0.000000), image_out);
    out1 = image_out;
}
...
void NG_gltf_normalmap_vector3_1_0(sampler2D file, vec3 default1, vec2 texcoord1, vec2 pivot, vec2 scale, float rotate, vec2 offset, int operationorder, int uaddressmode, int vaddressmode, int filtertype, out vec3 out1)
{
    ...
    mx_normalmap_float(image_out, 1.000000, geomprop_Nworld_out1, geomprop_Tworld_out1, geomprop_Bworld_out1, normalmap_out);
    out1 = normalmap_out;
}
...
void NG_NG_gltf_image_color3_color3_1_0(sampler2D file, vec3 factor, vec3 default1, vec2 texcoord1, vec2 pivot, vec2 scale, float rotate, vec2 offset, int operationorder, int uaddressmode, int vaddressmode, int filtertype, out vec3 out1)
{
    const vec2 invert_scale_in1_tmp = vec2(1.000000, 1.000000);
    vec2 invert_scale_out = invert_scale_in1_tmp / scale;
    const float negate_rotate_in2_tmp = -1.000000;
    ...
    NG_place2d_vector2(texcoord1, pivot, invert_scale_out, negate_rotate_out, negate_offset_out, operationorder, place2d_out);
    vec3 image_out = vec3(0.0);
    mx_image_color3(file, 0, default1, place2d_out, uaddressmode, vaddressmode, filtertype, 0, 0, 0, vec2(1.000000, 1.000000), vec2(0.000000, 0.000000), image_out);
    vec3 scale_image_out = factor * image_out;
    out1 = scale_image_out;
}
...
void IMPL_gltf_pbr_surfaceshader(vec3 base_color, float metallic, float roughness, vec3 normal, vec3 tangent, float occlusion, float transmission, float specular, vec3 specular_color, float ior, float alpha, int alpha_mode, float alpha_cutoff, float iridescence, float iridescence_ior, float iridescence_thickness, vec3 sheen_color, float sheen_roughness, float clearcoat, float clearcoat_roughness, vec3 clearcoat_normal, vec3 emissive, float emissive_strength, float thickness, float attenuation_distance, vec3 attenuation_color, out surfaceshader out1)
{
    ...
    surfaceshader shader_constructor_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            mx_dielectric_bsdf(closureData, clearcoat, vec3(1.000000, 1.000000, 1.000000), 1.500000, clearcoat_roughness_uv_out, 0.000000, 1.500000, clearcoat_normal, tangent, 0, 0, clearcoat_bsdf_out);
            ...
            mx_generalized_schlick_bsdf(closureData, 1.000000, base_color, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 5.000000, roughness_uv_out, iridescence_thickness, iridescence_ior, normal, tangent, 0, 0, tf_metal_bsdf_out);
            BSDF metal_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf(closureData, 1.000000, base_color, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 5.000000, roughness_uv_out, 0.000000, 1.500000, normal, tangent, 0, 0, metal_bsdf_out);
            BSDF mix_iridescent_metal_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, tf_metal_bsdf_out, metal_bsdf_out, iridescence, mix_iridescent_metal_bsdf_out);
            BSDF tf_reflection_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf(closureData, 1.000000, dielectric_f0_out, vec3(1.000000, 1.000000, 1.000000), dielectric_f90_out, 5.000000, roughness_uv_out, iridescence_thickness, iridescence_ior, normal, tangent, 0, 0, tf_reflection_bsdf_out);
            BSDF transmission_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_dielectric_bsdf(closureData, 1.000000, base_color, ior, roughness_uv_out, 0.000000, 1.500000, normal, tangent, 0, 1, transmission_bsdf_out);
            BSDF diffuse_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, 1.000000, base_color, 0.000000, normal, false, diffuse_bsdf_out);
            BSDF transmission_mix_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, transmission_bsdf_out, diffuse_bsdf_out, transmission, transmission_mix_out);
            BSDF tf_dielectric_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, tf_reflection_bsdf_out, transmission_mix_out, tf_dielectric_bsdf_out);
            BSDF reflection_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_generalized_schlick_bsdf(closureData, 1.000000, dielectric_f0_out, vec3(1.000000, 1.000000, 1.000000), dielectric_f90_out, 5.000000, roughness_uv_out, 0.000000, 1.500000, normal, tangent, 0, 0, reflection_bsdf_out);
            BSDF dielectric_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, reflection_bsdf_out, transmission_mix_out, dielectric_bsdf_out);
            BSDF mix_iridescent_dielectric_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, tf_dielectric_bsdf_out, dielectric_bsdf_out, iridescence, mix_iridescent_dielectric_bsdf_out);
            BSDF base_mix_out = BSDF(vec3(0.0),vec3(1.0));
            mx_mix_bsdf(closureData, mix_iridescent_metal_bsdf_out, mix_iridescent_dielectric_bsdf_out, metallic, base_mix_out);
            BSDF sheen_layer_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, sheen_bsdf_out, base_mix_out, sheen_layer_out);
...
void main()
{
    ...
    vec3 image_basecolor_outcolor = vec3(0.0);
    float image_basecolor_outa = 0.0;
    NG_gltf_colorimage(image_basecolor_file, image_basecolor_default, geomprop_UV0_out1, image_basecolor_pivot, image_basecolor_scale, image_basecolor_rotate, image_basecolor_offset, image_basecolor_operationorder, image_basecolor_uaddressmode, image_basecolor_vaddressmode, image_basecolor_filtertype, image_basecolor_color, image_basecolor_geomcolor, image_basecolor_outcolor, image_basecolor_outa);
    vec3 image_orm_out = vec3(0.0);
    NG_gltf_image_vector3_vector3_1_0(image_orm_file, image_orm_default, geomprop_UV0_out1, image_orm_pivot, image_orm_scale, image_orm_rotate, image_orm_offset, image_orm_operationorder, image_orm_uaddressmode, image_orm_vaddressmode, image_orm_filtertype, image_orm_out);
    vec3 image_normal_out = vec3(0.0);
    NG_gltf_normalmap_vector3_1_0(image_normal_file, image_normal_default, geomprop_UV0_out1, image_normal_pivot, image_normal_scale, image_normal_rotate, image_normal_offset, image_normal_operationorder, image_normal_uaddressmode, image_normal_vaddressmode, image_normal_filtertype, image_normal_out);
    vec3 image_emission_out = vec3(0.0);
    NG_NG_gltf_image_color3_color3_1_0(image_emission_file, image_emission_factor, image_emission_default, geomprop_UV0_out1, image_emission_pivot, image_emission_scale, image_emission_rotate, image_emission_offset, image_emission_operationorder, image_emission_uaddressmode, image_emission_vaddressmode, image_emission_filtertype, image_emission_out);
    vec3 image_basecolor_outcolor_cm_out = vec3(0.0);
    NG_srgb_texture_to_lin_rec709_color3(image_basecolor_outcolor, image_basecolor_outcolor_cm_out);
    float image_orm_channels_outx = 0.0;
    float image_orm_channels_outy = 0.0;
    float image_orm_channels_outz = 0.0;
    NG_separate3_vector3(image_orm_out, image_orm_channels_outx, image_orm_channels_outy, image_orm_channels_outz);
    vec3 image_emission_out_cm_out = vec3(0.0);
    NG_srgb_texture_to_lin_rec709_color3(image_emission_out, image_emission_out_cm_out);
    surfaceshader SR_boombox_out = surfaceshader(vec3(0.0),vec3(0.0));
    IMPL_gltf_pbr_surfaceshader(image_basecolor_outcolor_cm_out, image_orm_channels_outz, image_orm_channels_outy, image_normal_out, geomprop_Tworld_out1, image_orm_channels_outx, SR_boombox_transmission, SR_boombox_specular, SR_boombox_specular_color, SR_boombox_ior, image_basecolor_outa, SR_boombox_alpha_mode, SR_boombox_alpha_cutoff, SR_boombox_iridescence, SR_boombox_iridescence_ior, SR_boombox_iridescence_thickness, SR_boombox_sheen_color, SR_boombox_sheen_roughness, SR_boombox_clearcoat, SR_boombox_clearcoat_roughness, geomprop_Nworld_out1, image_emission_out_cm_out, SR_boombox_emissive_strength, SR_boombox_thickness, SR_boombox_attenuation_distance, SR_boombox_attenuation_color, SR_boombox_out);
    material Material_boombox_out = SR_boombox_out;
    float outAlpha = clamp(1.0 - dot(Material_boombox_out.transparency, vec3(0.3333)), 0.0, 1.0);
    out1 = vec4(Material_boombox_out.color, outAlpha);
