...
uniform vec3 LamaEmission1_color_cm_in = vec3(1.000000, 1.000000, 0.000000);
uniform vec3 LamaEmission2_color_cm_in = vec3(1.000000, 0.000000, 1.000000);
uniform float LamaAddEDF_weight1 = 0.500000;
uniform float LamaAddEDF_weight2 = 0.500000;
uniform float LamaAddEDFSurface_opacity = 1.000000;
uniform bool LamaAddEDFSurface_thin_walled = false;
...
void IMPL_lama_emission(ClosureData closureData, vec3 color1, out EDF out1)
{
    EDF emission_out = EDF(0.0);
    mx_uniform_edf(closureData, color1, emission_out);
    out1 = emission_out;
}
...
void NG_lama_add_edf(ClosureData closureData, EDF material1, EDF material2, float weight1, float weight2, out EDF out1)
{
    EDF mul1_out = EDF(0.0);
    mx_multiply_edf_float(closureData, material1, weight1, mul1_out);
    EDF mul2_out = EDF(0.0);
    mx_multiply_edf_float(closureData, material2, weight2, mul2_out);
    EDF add1_out = EDF(0.0);
    mx_add_edf(closureData, mul1_out, mul2_out, add1_out);
    out1 = add1_out;
}

void main()
{
    vec3 LamaEmission1_color_cm_out = vec3(0.0);
    NG_acescg_to_lin_rec709_color3(LamaEmission1_color_cm_in, LamaEmission1_color_cm_out);
    vec3 LamaEmission2_color_cm_out = vec3(0.0);
    NG_acescg_to_lin_rec709_color3(LamaEmission2_color_cm_in, LamaEmission2_color_cm_out);
    surfaceshader LamaAddEDFSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add surface emission
        {
            ...
            EDF LamaEmission1_out = EDF(0.0);
            IMPL_lama_emission(closureData, LamaEmission1_color_cm_out, LamaEmission1_out);
            EDF LamaEmission2_out = EDF(0.0);
            IMPL_lama_emission(closureData, LamaEmission2_color_cm_out, LamaEmission2_out);
            EDF LamaAddEDF_out = EDF(0.0);
            NG_lama_add_edf(closureData, LamaEmission1_out, LamaEmission2_out, LamaAddEDF_weight1, LamaAddEDF_weight2, LamaAddEDF_out);
            LamaAddEDFSurface_out.color += LamaAddEDF_out;
        }

    }

    material LamaAddEDFTest_out = LamaAddEDFSurface_out;
    out1 = vec4(LamaAddEDFTest_out.color, 1.0);
}
