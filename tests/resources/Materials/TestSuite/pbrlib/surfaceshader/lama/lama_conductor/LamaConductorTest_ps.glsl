...
uniform vec3 LamaConductor_edgeColor_cm_in = vec3(0.713700, 0.737300, 0.455000);
...
uniform float LamaConductorSurface_opacity = 1.000000;
uniform bool LamaConductorSurface_thin_walled = false;
...
void main()
{
    ...
    surfaceshader LamaConductorSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = LamaConductorSurface_opacity;
        ...
        // Add environment contribution
        {
            ...
            LamaConductorSurface_out.color += occlusion * LamaConductor_out.response;
        }
        ...
        BSDF LamaConductor_out = BSDF(vec3(0.0),vec3(1.0));
        IMPL_lama_conductor(closureData, LamaConductor_tint_cm_out, LamaConductor_fresnelMode, LamaConductor_IOR, LamaConductor_extinction, LamaConductor_reflectivity_cm_out, LamaConductor_edgeColor_cm_out, LamaConductor_roughness, geomprop_Nworld_out1, LamaConductor_anisotropy, geomprop_Tworld_out1, LamaConductor_anisotropyRotation, LamaConductor_out);
        LamaConductorSurface_out.color += LamaConductor_out.response;

        // Compute and apply surface opacity
        {
            LamaConductorSurface_out.color *= surfaceOpacity;
            LamaConductorSurface_out.transparency = mix(vec3(1.0), LamaConductorSurface_out.transparency, surfaceOpacity);
        }
    }

    material LamaConductorTest_out = LamaConductorSurface_out;
    out1 = vec4(LamaConductorTest_out.color, 1.0);
}
