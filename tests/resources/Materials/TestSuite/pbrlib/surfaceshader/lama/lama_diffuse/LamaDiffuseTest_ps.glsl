...
uniform vec3 LamaDiffuse_color_cm_in = vec3(0.180000, 0.180000, 0.180000);
uniform float LamaDiffuse_roughness = 0.500000;
uniform float LamaDiffuse_energyCompensation = 1.000000;
uniform float LamaDiffuseSurface_opacity = 1.000000;
uniform bool LamaDiffuseSurface_thin_walled = false;
...
void NG_lama_diffuse(ClosureData closureData, vec3 color1, float roughness, vec3 normal, float energyCompensation, out BSDF out1)
{
    float roughness_squared_out = roughness * roughness;
    const float half_roughness_squared_in2_tmp = 0.500000;
    float half_roughness_squared_out = roughness_squared_out * half_roughness_squared_in2_tmp;
    BSDF oren_nayar_compensated_out = BSDF(vec3(0.0),vec3(1.0));
    mx_oren_nayar_diffuse_bsdf(closureData, 1.000000, color1, half_roughness_squared_out, normal, true, oren_nayar_compensated_out);
    BSDF oren_nayar_uncompensated_out = BSDF(vec3(0.0),vec3(1.0));
    mx_oren_nayar_diffuse_bsdf(closureData, 1.000000, color1, half_roughness_squared_out, normal, false, oren_nayar_uncompensated_out);
    BSDF oren_nayar_out = BSDF(vec3(0.0),vec3(1.0));
    mx_mix_bsdf(closureData, oren_nayar_compensated_out, oren_nayar_uncompensated_out, energyCompensation, oren_nayar_out);
    out1 = oren_nayar_out;
}
...
void main()
{
    ...
    vec3 LamaDiffuse_color_cm_out = vec3(0.0);
    NG_acescg_to_lin_rec709_color3(LamaDiffuse_color_cm_in, LamaDiffuse_color_cm_out);
    surfaceshader LamaDiffuseSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = LamaDiffuseSurface_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF LamaDiffuse_out = BSDF(vec3(0.0),vec3(1.0));
            NG_lama_diffuse(closureData, LamaDiffuse_color_cm_out, LamaDiffuse_roughness, geomprop_Nworld_out1, LamaDiffuse_energyCompensation, LamaDiffuse_out);

            LamaDiffuseSurface_out.color += occlusion * LamaDiffuse_out.response;
        }
        ...
        BSDF LamaDiffuse_out = BSDF(vec3(0.0),vec3(1.0));
        NG_lama_diffuse(closureData, LamaDiffuse_color_cm_out, LamaDiffuse_roughness, geomprop_Nworld_out1, LamaDiffuse_energyCompensation, LamaDiffuse_out);
        LamaDiffuseSurface_out.color += LamaDiffuse_out.response;

        // Compute and apply surface opacity
        {
            LamaDiffuseSurface_out.color *= surfaceOpacity;
            LamaDiffuseSurface_out.transparency = mix(vec3(1.0), LamaDiffuseSurface_out.transparency, surfaceOpacity);
        }
    }

    material LamaDiffuseTest_out = LamaDiffuseSurface_out;
    out1 = vec4(LamaDiffuseTest_out.color, 1.0);
}
