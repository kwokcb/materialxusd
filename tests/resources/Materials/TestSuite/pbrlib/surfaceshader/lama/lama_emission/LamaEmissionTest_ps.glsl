...
uniform vec3 LamaEmission_color_cm_in = vec3(0.000000, 0.000000, 0.000000);
uniform float LamaEmissionSurface_opacity = 1.000000;
uniform bool LamaEmissionSurface_thin_walled = false;
...
void main()
{
    vec3 LamaEmission_color_cm_out = vec3(0.0);
    NG_acescg_to_lin_rec709_color3(LamaEmission_color_cm_in, LamaEmission_color_cm_out);
    surfaceshader LamaEmissionSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add surface emission
        {
            ...
            EDF LamaEmission_out = EDF(0.0);
            IMPL_lama_emission(closureData, LamaEmission_color_cm_out, LamaEmission_out);
            LamaEmissionSurface_out.color += LamaEmission_out;
        }

    }

    material LamaEmissionTest_out = LamaEmissionSurface_out;
    out1 = vec4(LamaEmissionTest_out.color, 1.0);
}
