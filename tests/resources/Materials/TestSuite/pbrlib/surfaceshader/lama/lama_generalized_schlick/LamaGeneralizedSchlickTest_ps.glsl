...
void IMPL_lama_generalized_schlick(ClosureData closureData, vec3 reflectionTint, vec3 transmissionTint, int fresnelMode, vec3 IOR, vec3 reflectivity, vec3 reflectivity90, float reflectivityProfile, float roughness, vec3 normal, float anisotropy, vec3 anisotropyDirection, float anisotropyRotation, vec3 absorptionColor, float absorptionRadius, vec3 scatterColor, float scatterAnisotropy, out BSDF out1)
{
    ...
    mx_generalized_schlick_bsdf(closureData, 1.000000, fresnel_mode_switch_out, vec3(1.000000, 1.000000, 1.000000), reflectivity90, exponent_out, roughness_anisotropic_squared_clamped_out, 0.000000, 1.500000, normal, tangent_rotate_normalize_out, 0, 0, reflection_generalized_schlick_bsdf_out);
    ...
    mx_generalized_schlick_bsdf(closureData, 1.000000, fresnel_mode_switch_out, vec3(1.000000, 1.000000, 1.000000), reflectivity90, exponent_out, roughness_anisotropic_squared_clamped_out, 0.000000, 1.500000, normal, tangent_rotate_normalize_out, 0, 1, transmission_bsdf_out);
...
void main()
{
    ...
    surfaceshader LamaGeneralizedSchlickSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Compute and apply surface opacity
        {
            ...
            LamaGeneralizedSchlickSurface_out.transparency = mix(vec3(1.0), LamaGeneralizedSchlickSurface_out.transparency, surfaceOpacity);
        }
    }
