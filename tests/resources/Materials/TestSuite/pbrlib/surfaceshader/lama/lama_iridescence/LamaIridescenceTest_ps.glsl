...
uniform float LamaIridescence_roughness = 0.000000;
uniform float LamaIridescence_anisotropy = 0.000000;
uniform float LamaIridescence_anisotropyRotation = 0.000000;
uniform float LamaIridescence_relativeFilmThickness = 0.500000;
uniform float LamaIridescence_minFilmThickness = 400.000000;
uniform float LamaIridescence_maxFilmThickness = 800.000000;
uniform float LamaIridescence_filmIOR = 1.300000;
uniform float LamaIridescenceSurface_opacity = 1.000000;
uniform bool LamaIridescenceSurface_thin_walled = false;
...
void IMPL_lama_iridescence(ClosureData closureData, float roughness, float anisotropy, vec3 anisotropyDirection, float anisotropyRotation, float relativeFilmThickness, float minFilmThickness, float maxFilmThickness, float filmIOR, out BSDF out1)
{
    ...
    vec3 tangent_normal_out = normalize(vd.normalWorld);
    ...
    BSDF iridescence_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
    mx_dielectric_bsdf(closureData, 1.000000, vec3(1.000000, 1.000000, 1.000000), 1.000000, roughness_anisotropic_squared_clamped_out, thinfilm_thickness_out, filmIOR, geomprop_Nworld_out1, tangent_rotate_normalize_out, 0, 2, iridescence_bsdf_out);
    out1 = iridescence_bsdf_out;
}

void main()
{
    vec3 geomprop_Tworld_out1 = normalize(vd.tangentWorld);
    surfaceshader LamaIridescenceSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = LamaIridescenceSurface_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF LamaIridescence_out = BSDF(vec3(0.0),vec3(1.0));
            IMPL_lama_iridescence(closureData, LamaIridescence_roughness, LamaIridescence_anisotropy, geomprop_Tworld_out1, LamaIridescence_anisotropyRotation, LamaIridescence_relativeFilmThickness, LamaIridescence_minFilmThickness, LamaIridescence_maxFilmThickness, LamaIridescence_filmIOR, LamaIridescence_out);

            LamaIridescenceSurface_out.color += occlusion * LamaIridescence_out.response;
        }
        ...
        BSDF LamaIridescence_out = BSDF(vec3(0.0),vec3(1.0));
        IMPL_lama_iridescence(closureData, LamaIridescence_roughness, LamaIridescence_anisotropy, geomprop_Tworld_out1, LamaIridescence_anisotropyRotation, LamaIridescence_relativeFilmThickness, LamaIridescence_minFilmThickness, LamaIridescence_maxFilmThickness, LamaIridescence_filmIOR, LamaIridescence_out);
        LamaIridescenceSurface_out.color += LamaIridescence_out.response;

        // Compute and apply surface opacity
        {
            LamaIridescenceSurface_out.color *= surfaceOpacity;
            LamaIridescenceSurface_out.transparency = mix(vec3(1.0), LamaIridescenceSurface_out.transparency, surfaceOpacity);
        }
    }

    material LamaIridescenceTest_out = LamaIridescenceSurface_out;
    out1 = vec4(LamaIridescenceTest_out.color, 1.0);
}
