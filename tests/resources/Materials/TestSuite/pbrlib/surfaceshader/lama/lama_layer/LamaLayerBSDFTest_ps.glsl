...
uniform float LamaLayerBSDF_topMix = 1.000000;
uniform float LamaLayerBSDF_topThickness = 0.000000;
uniform float LamaLayerBSDFSurface_opacity = 1.000000;
uniform bool LamaLayerBSDFSurface_thin_walled = false;
...
void NG_lama_layer_bsdf(ClosureData closureData, BSDF materialTop, BSDF materialBase, float topMix, float topThickness, out BSDF out1)
{
    BSDF mul_out = BSDF(vec3(0.0),vec3(1.0));
    mx_multiply_bsdf_float(closureData, materialTop, topMix, mul_out);
    BSDF layer_out = BSDF(vec3(0.0),vec3(1.0));
    mx_layer_bsdf(closureData, mul_out, materialBase, layer_out);
    out1 = layer_out;
}
...
void main()
{
    ...
    surfaceshader LamaLayerBSDFSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = LamaLayerBSDFSurface_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF LamaLayerBSDF_out = BSDF(vec3(0.0),vec3(1.0));
            NG_lama_layer_bsdf(closureData, LamaDielectric_out, LamaConductor_out, LamaLayerBSDF_topMix, LamaLayerBSDF_topThickness, LamaLayerBSDF_out);

            LamaLayerBSDFSurface_out.color += occlusion * LamaLayerBSDF_out.response;
        }
        ...
        BSDF LamaLayerBSDF_out = BSDF(vec3(0.0),vec3(1.0));
        NG_lama_layer_bsdf(closureData, LamaDielectric_out, LamaConductor_out, LamaLayerBSDF_topMix, LamaLayerBSDF_topThickness, LamaLayerBSDF_out);
        LamaLayerBSDFSurface_out.color += LamaLayerBSDF_out.response;

        // Compute and apply surface opacity
        {
            LamaLayerBSDFSurface_out.color *= surfaceOpacity;
            LamaLayerBSDFSurface_out.transparency = mix(vec3(1.0), LamaLayerBSDFSurface_out.transparency, surfaceOpacity);
        }
    }

    material LamaLayerBSDFTest_out = LamaLayerBSDFSurface_out;
    out1 = vec4(LamaLayerBSDFTest_out.color, 1.0);
}
