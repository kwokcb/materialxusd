...
uniform float LamaMixBSDF_mix = 0.500000;
uniform float LamaMixBSDFSurface_opacity = 1.000000;
uniform bool LamaMixBSDFSurface_thin_walled = false;
...
void NG_lama_mix_bsdf(ClosureData closureData, BSDF material1, BSDF material2, float mix1, out BSDF out1)
{
    BSDF mix_out = BSDF(vec3(0.0),vec3(1.0));
    mx_mix_bsdf(closureData, material2, material1, mix1, mix_out);
    out1 = mix_out;
}
...
void main()
{
    ...
    surfaceshader LamaMixBSDFSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = LamaMixBSDFSurface_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF LamaMixBSDF_out = BSDF(vec3(0.0),vec3(1.0));
            NG_lama_mix_bsdf(closureData, LamaConductor_out, LamaDielectric_out, LamaMixBSDF_mix, LamaMixBSDF_out);

            LamaMixBSDFSurface_out.color += occlusion * LamaMixBSDF_out.response;
        }
        ...
        BSDF LamaMixBSDF_out = BSDF(vec3(0.0),vec3(1.0));
        NG_lama_mix_bsdf(closureData, LamaConductor_out, LamaDielectric_out, LamaMixBSDF_mix, LamaMixBSDF_out);
        LamaMixBSDFSurface_out.color += LamaMixBSDF_out.response;

        // Compute and apply surface opacity
        {
            LamaMixBSDFSurface_out.color *= surfaceOpacity;
            LamaMixBSDFSurface_out.transparency = mix(vec3(1.0), LamaMixBSDFSurface_out.transparency, surfaceOpacity);
        }
    }

    material LamaMixBSDFTest_out = LamaMixBSDFSurface_out;
    out1 = vec4(LamaMixBSDFTest_out.color, 1.0);
}
