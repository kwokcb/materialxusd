...
uniform float LamaMixEDF_mix = 0.500000;
uniform float LamaMixEDFSurface_opacity = 1.000000;
uniform bool LamaMixEDFSurface_thin_walled = false;
...
void NG_lama_mix_edf(ClosureData closureData, EDF material1, EDF material2, float mix1, out EDF out1)
{
    EDF mix_out = EDF(0.0);
    mx_mix_edf(closureData, material2, material1, mix1, mix_out);
    out1 = mix_out;
}
...
void main()
{
    ...
    surfaceshader LamaMixEDFSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add surface emission
        {
            ...
            EDF LamaMixEDF_out = EDF(0.0);
            NG_lama_mix_edf(closureData, LamaEmission1_out, LamaEmission2_out, LamaMixEDF_mix, LamaMixEDF_out);
            LamaMixEDFSurface_out.color += LamaMixEDF_out;
        }

    }

    material LamaMixEDFTest_out = LamaMixEDFSurface_out;
    out1 = vec4(LamaMixEDFTest_out.color, 1.0);
}
