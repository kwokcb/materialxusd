...
uniform vec3 LamaSheen_color_cm_in = vec3(1.000000, 1.000000, 1.000000);
uniform float LamaSheen_roughness = 0.100000;
uniform float LamaSheenSurface_opacity = 1.000000;
uniform bool LamaSheenSurface_thin_walled = false;
...
void IMPL_lama_sheen(ClosureData closureData, vec3 color1, float roughness, vec3 normal, out BSDF out1)
{
    const float roughness_compressed_in2_tmp = 0.900000;
    float roughness_compressed_out = roughness * roughness_compressed_in2_tmp;
    const float roughness_remapped_in2_tmp = 0.100000;
    float roughness_remapped_out = roughness_compressed_out + roughness_remapped_in2_tmp;
    const float roughness_squared_in2_tmp = 2.000000;
    ...
    mx_sheen_bsdf(closureData, 1.000000, color1, roughness_squared_out, normal, 0, sheen_bsdf_out);
    out1 = sheen_bsdf_out;
}
...
void main()
{
    ...
    vec3 LamaSheen_color_cm_out = vec3(0.0);
    NG_acescg_to_lin_rec709_color3(LamaSheen_color_cm_in, LamaSheen_color_cm_out);
    surfaceshader LamaSheenSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = LamaSheenSurface_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF LamaSheen_out = BSDF(vec3(0.0),vec3(1.0));
            IMPL_lama_sheen(closureData, LamaSheen_color_cm_out, LamaSheen_roughness, geomprop_Nworld_out1, LamaSheen_out);

            LamaSheenSurface_out.color += occlusion * LamaSheen_out.response;
        }
        ...
        BSDF LamaSheen_out = BSDF(vec3(0.0),vec3(1.0));
        IMPL_lama_sheen(closureData, LamaSheen_color_cm_out, LamaSheen_roughness, geomprop_Nworld_out1, LamaSheen_out);
        LamaSheenSurface_out.color += LamaSheen_out.response;

        // Compute and apply surface opacity
        {
            LamaSheenSurface_out.color *= surfaceOpacity;
            LamaSheenSurface_out.transparency = mix(vec3(1.0), LamaSheenSurface_out.transparency, surfaceOpacity);
        }
    }

    material LamaSheenTest_out = LamaSheenSurface_out;
    out1 = vec4(LamaSheenTest_out.color, 1.0);
}
