...
uniform vec3 LamaSSS_color_cm_in = vec3(0.180000, 0.180000, 0.180000);
uniform vec3 LamaSSS_sssRadius_cm_in = vec3(0.000000, 0.000000, 0.000000);
uniform float LamaSSS_sssScale = 1.000000;
uniform int LamaSSS_sssMode = 0;
uniform float LamaSSS_sssIOR = 1.000000;
uniform float LamaSSS_sssAnisotropy = 0.000000;
uniform float LamaSSS_sssBleed = 0.000000;
uniform float LamaSSS_sssFollowTopology = 0.000000;
uniform int LamaSSS_sssSubset = 0;
uniform int LamaSSS_sssContinuationRays = 0;
uniform float LamaSSS_sssUnitLength = 0.003280;
uniform int LamaSSS_mode = 0;
uniform int LamaSSS_albedoInversionMethod = 0;
uniform float LamaSSSSurface_opacity = 1.000000;
uniform bool LamaSSSSurface_thin_walled = false;
...
void IMPL_lama_sss(ClosureData closureData, vec3 color1, vec3 normal, vec3 sssRadius, float sssScale, int sssMode, float sssIOR, float sssAnisotropy, float sssBleed, float sssFollowTopology, int sssSubset, int sssContinuationRays, float sssUnitLength, int mode, int albedoInversionMethod, out BSDF out1)
{
    vec3 subsurface_radius_scaled_out = sssRadius * sssScale;
    vec3 subsurface_multiply_unitlength_out = subsurface_radius_scaled_out * sssUnitLength;
    BSDF subsurface_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
    mx_subsurface_bsdf(closureData, 1.000000, color1, subsurface_multiply_unitlength_out, sssAnisotropy, normal, subsurface_bsdf_out);
    out1 = subsurface_bsdf_out;
}

void main()
{
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    vec3 LamaSSS_color_cm_out = vec3(0.0);
    NG_acescg_to_lin_rec709_color3(LamaSSS_color_cm_in, LamaSSS_color_cm_out);
    vec3 LamaSSS_sssRadius_cm_out = vec3(0.0);
    NG_acescg_to_lin_rec709_color3(LamaSSS_sssRadius_cm_in, LamaSSS_sssRadius_cm_out);
    surfaceshader LamaSSSSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = LamaSSSSurface_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF LamaSSS_out = BSDF(vec3(0.0),vec3(1.0));
            IMPL_lama_sss(closureData, LamaSSS_color_cm_out, geomprop_Nworld_out1, LamaSSS_sssRadius_cm_out, LamaSSS_sssScale, LamaSSS_sssMode, LamaSSS_sssIOR, LamaSSS_sssAnisotropy, LamaSSS_sssBleed, LamaSSS_sssFollowTopology, LamaSSS_sssSubset, LamaSSS_sssContinuationRays, LamaSSS_sssUnitLength, LamaSSS_mode, LamaSSS_albedoInversionMethod, LamaSSS_out);

            LamaSSSSurface_out.color += occlusion * LamaSSS_out.response;
        }
        ...
        BSDF LamaSSS_out = BSDF(vec3(0.0),vec3(1.0));
        IMPL_lama_sss(closureData, LamaSSS_color_cm_out, geomprop_Nworld_out1, LamaSSS_sssRadius_cm_out, LamaSSS_sssScale, LamaSSS_sssMode, LamaSSS_sssIOR, LamaSSS_sssAnisotropy, LamaSSS_sssBleed, LamaSSS_sssFollowTopology, LamaSSS_sssSubset, LamaSSS_sssContinuationRays, LamaSSS_sssUnitLength, LamaSSS_mode, LamaSSS_albedoInversionMethod, LamaSSS_out);
        LamaSSSSurface_out.color += LamaSSS_out.response;

        // Compute and apply surface opacity
        {
            LamaSSSSurface_out.color *= surfaceOpacity;
            LamaSSSSurface_out.transparency = mix(vec3(1.0), LamaSSSSurface_out.transparency, surfaceOpacity);
        }
    }

    material LamaSSSTest_out = LamaSSSSurface_out;
    out1 = vec4(LamaSSSTest_out.color, 1.0);
}
