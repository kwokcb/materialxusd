...
uniform vec3 LamaTranslucent_color_cm_in = vec3(0.180000, 0.180000, 0.180000);
uniform float LamaTranslucent_roughness = 0.000000;
uniform float LamaTranslucent_energyCompensation = 1.000000;
uniform float LamaTranslucentSurface_opacity = 1.000000;
uniform bool LamaTranslucentSurface_thin_walled = false;
...
void NG_lama_translucent(ClosureData closureData, vec3 color1, float roughness, vec3 normal, float energyCompensation, out BSDF out1)
{
    BSDF translucent_bsdf1_out = BSDF(vec3(0.0),vec3(1.0));
    mx_translucent_bsdf(closureData, 1.000000, color1, normal, translucent_bsdf1_out);
    out1 = translucent_bsdf1_out;
}

void main()
{
    vec3 geomprop_Nworld_out1 = normalize(vd.normalWorld);
    vec3 LamaTranslucent_color_cm_out = vec3(0.0);
    NG_acescg_to_lin_rec709_color3(LamaTranslucent_color_cm_in, LamaTranslucent_color_cm_out);
    surfaceshader LamaTranslucentSurface_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        float surfaceOpacity = LamaTranslucentSurface_opacity;
        ...
        // Add environment contribution
        {
            ...
            BSDF LamaTranslucent_out = BSDF(vec3(0.0),vec3(1.0));
            NG_lama_translucent(closureData, LamaTranslucent_color_cm_out, LamaTranslucent_roughness, geomprop_Nworld_out1, LamaTranslucent_energyCompensation, LamaTranslucent_out);

            LamaTranslucentSurface_out.color += occlusion * LamaTranslucent_out.response;
        }
        ...
        BSDF LamaTranslucent_out = BSDF(vec3(0.0),vec3(1.0));
        NG_lama_translucent(closureData, LamaTranslucent_color_cm_out, LamaTranslucent_roughness, geomprop_Nworld_out1, LamaTranslucent_energyCompensation, LamaTranslucent_out);
        LamaTranslucentSurface_out.color += LamaTranslucent_out.response;

        // Compute and apply surface opacity
        {
            LamaTranslucentSurface_out.color *= surfaceOpacity;
            LamaTranslucentSurface_out.transparency = mix(vec3(1.0), LamaTranslucentSurface_out.transparency, surfaceOpacity);
        }
    }

    material LamaTranslucentTest_out = LamaTranslucentSurface_out;
    out1 = vec4(LamaTranslucentTest_out.color, 1.0);
}
