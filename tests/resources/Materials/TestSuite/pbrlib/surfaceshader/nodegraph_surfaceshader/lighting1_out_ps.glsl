...
uniform float diffusebsdf1_weight = 1.000000;
uniform vec3 diffusebsdf1_color = vec3(0.300000, 0.300000, 1.000000);
uniform float diffusebsdf1_roughness = 0.800000;
uniform bool diffusebsdf1_energy_compensation = false;
...
void main()
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF diffusebsdf1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, diffusebsdf1_weight, diffusebsdf1_color, diffusebsdf1_roughness, geomprop_Nworld_out1, diffusebsdf1_energy_compensation, diffusebsdf1_out);

            surface1_out.color += occlusion * diffusebsdf1_out.response;
        }

        // Calculate the BSDF transmission for viewing direction
        BSDF diffusebsdf1_out = BSDF(vec3(0.0),vec3(1.0));
        surface1_out.color += diffusebsdf1_out.response;
