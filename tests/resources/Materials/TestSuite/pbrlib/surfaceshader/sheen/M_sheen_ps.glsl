...
uniform float SR_sheen_diffuse = 1.000000;
uniform vec3 SR_sheen_diffuse_color = vec3(0.000000, 0.000000, 0.950000);
uniform float SR_sheen_sheen = 1.000000;
uniform vec3 SR_sheen_sheen_color = vec3(0.516000, 0.524000, 0.778000);
uniform float SR_sheen_sheen_roughness = 0.300000;
...
void IMP_sheen_surface(float diffuse, vec3 diffuse_color, float sheen, vec3 sheen_color, float sheen_roughness, out surfaceshader out1)
{
    ...
    surfaceshader surface1_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        // Add environment contribution
        {
            ...
            BSDF sheen1_layer_top_out = BSDF(vec3(0.0),vec3(1.0));
            mx_sheen_bsdf(closureData, sheen, sheen_color, sheen_roughness, geomprop_Nworld_out1, 0, sheen1_layer_top_out);
            BSDF diffuse1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_oren_nayar_diffuse_bsdf(closureData, diffuse, diffuse_color, 0.000000, geomprop_Nworld_out1, false, diffuse1_out);
            BSDF sheen1_out = BSDF(vec3(0.0),vec3(1.0));
            mx_layer_bsdf(closureData, sheen1_layer_top_out, diffuse1_out, sheen1_out);

            surface1_out.color += occlusion * sheen1_out.response;
        }
        ...
        BSDF sheen1_out = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, sheen1_layer_top_out, diffuse1_out, sheen1_out);
        surface1_out.color += sheen1_out.response;

        // Compute and apply surface opacity
        {
            surface1_out.color *= surfaceOpacity;
            surface1_out.transparency = mix(vec3(1.0), surface1_out.transparency, surfaceOpacity);
        }
    }
...
void main()
{
    surfaceshader SR_sheen_out = surfaceshader(vec3(0.0),vec3(0.0));
    ...
    material M_sheen_out = SR_sheen_out;
    out1 = vec4(M_sheen_out.color, 1.0);
}
