...
uniform int N_texcoord_index = 0;
uniform vec2 N_multiply_in2 = vec2(8.000000, 8.000000);
uniform sampler2D N_texture_normal_file;
uniform int N_texture_normal_wrapS = 2;
uniform int N_texture_normal_wrapT = 2;
uniform vec4 N_texture_normal_fallback = vec4(0.000000, 0.000000, 0.000000, 1.000000);
uniform vec4 N_texture_normal_scale = vec4(1.000000, 1.000000, 1.000000, 1.000000);
uniform vec4 N_texture_normal_bias = vec4(0.000000, 0.000000, 0.000000, 0.000000);
uniform float N_normalmap_scale = 1.000000;
uniform vec3 N_surface_diffuseColor = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 N_surface_emissiveColor = vec3(0.000000, 0.000000, 0.000000);
uniform int N_surface_useSpecularWorkflow = 0;
uniform vec3 N_surface_specularColor = vec3(0.000000, 0.000000, 0.000000);
uniform float N_surface_metallic = 1.000000;
uniform float N_surface_roughness = 0.001000;
uniform float N_surface_clearcoat = 0.000000;
uniform float N_surface_clearcoatRoughness = 0.010000;
uniform float N_surface_opacity = 1.000000;
uniform int N_surface_opacityMode = 0;
uniform float N_surface_opacityThreshold = 0.000000;
uniform float N_surface_ior = 1.500000;
uniform float N_surface_displacement = 0.000000;
uniform float N_surface_occlusion = 1.000000;
...
void IMP_UsdUVTexture_23(sampler2D file, vec2 st, int wrapS, int wrapT, vec4 fallback, vec4 scale, vec4 bias, out float r, out float g, out float b, out float a, out vec3 rgb)
{
    vec4 image_reader_out = vec4(0.0);
    mx_image_color4(file, 0, fallback, st, wrapS, wrapT, 1, 0, 0, 0, vec2(1.000000, 1.000000), vec2(0.000000, 0.000000), image_reader_out);
    vec4 image_scale_out = image_reader_out * scale;
    vec4 image_bias_out = image_scale_out + bias;
    ...
    vec3 image_bias_color3_out = vec3(0.0);
...
void main()
{
    ...
    vec3 geomprop_Tworld_out1 = normalize(vd.tangentWorld);
    vec3 geomprop_Bworld_out1 = normalize(vd.bitangentWorld);
    vec2 N_texcoord_out = vd.texcoord_0.xy;
    vec2 N_multiply_out = N_texcoord_out * N_multiply_in2;
    float N_texture_normal_r = 0.0;
    float N_texture_normal_g = 0.0;
    float N_texture_normal_b = 0.0;
    float N_texture_normal_a = 0.0;
    vec3 N_texture_normal_rgb = vec3(0.0);
    IMP_UsdUVTexture_23(N_texture_normal_file, N_multiply_out, N_texture_normal_wrapS, N_texture_normal_wrapT, N_texture_normal_fallback, N_texture_normal_scale, N_texture_normal_bias, N_texture_normal_r, N_texture_normal_g, N_texture_normal_b, N_texture_normal_a, N_texture_normal_rgb);
    vec3 N_convert_out = vec3(0.0);
    NG_convert_color3_vector3(N_texture_normal_rgb, N_convert_out);
    vec3 N_normalmap_out = vec3(0.0);
    ...
    surfaceshader N_surface_out = surfaceshader(vec3(0.0),vec3(0.0));
    ...
    material N_material_out = N_surface_out;
    float outAlpha = clamp(1.0 - dot(N_material_out.transparency, vec3(0.3333)), 0.0, 1.0);
    out1 = vec4(N_material_out.color, outAlpha);
