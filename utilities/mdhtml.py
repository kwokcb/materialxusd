...
def convertToHtml(inputFileName, outputFileName, templateFileName=None, iframe=False, topFolder="..",
    ...
    if not iframe:
        ...
        if inputFileName.endswith('.md'):
            html = markdown.markdown(text, tab_length=2, extensions=['tables', 'codehilite', 'fenced_code'])
    ...
    if templateFileName:
        ...
        # Replace all help models with the content
        
        soup = BeautifulSoup(html, 'html.parser')

        # Find the DOM element by class
        help_modals = soup.find_all('div', class_='help_modal')

        # Iterate and print the content of each matching div
        for help_modal in help_modals:
        
            title = help_modal.get('title')  # Get the 'title' attribute
            button_title = help_modal.get('button_title')  # Get the 'button_title' attribute
            button_icon = help_modal.get('button_icon')  # Get the 'button_icon' attribute
            id = help_modal.get('id')  # Get the 'id' attribute
            content_file = help_modal.get('content_file')  # Get the 'content_file' attribute

            # Get parent path from inputFileName
            parentPath = os.path.dirname(inputFileName)
            if content_file:
                content_file = os.path.join(parentPath, content_file)

            # Get the execution path
            currentPath = os.path.dirname(os.path.realpath(__file__))
            templateFileName = os.path.join(currentPath, "help_template.html")
            new_content = build_help_from_template(content_file, templateFileName, id, title, button_title, button_icon)

            # Replace the help_modal with the help contents
            help_modal.replace_with(BeautifulSoup(new_content, 'html.parser'))            

        # Get the modified HTML
        if (len(help_modals) > 0):
            html = soup.prettify()
...
def main():
    ...
    for filePair in filePairs:
        ...
        if opts.popup:
            convertToHtml(filePair[0], filePair[1], opts.template, opts.iframe, opts.topfolder, opts.docfolder, '',
                        '',  opts.removeMermaid)
        else:
